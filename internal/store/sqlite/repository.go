...
func (s *EncounterStore) Get() (*domain.Encounter, error) {
	...
	for _, c := range combatantRows {
		combatants = append(combatants, domain.Combatant{
			...
			ResistPhysicalHead:      int(c.DefenseHead),
			ResistPhysicalTorso:     int(c.DefenseTorso),
			ResistPhysicalLeftArm:   int(c.DefenseLeftArm),
			ResistPhysicalRightArm:  int(c.DefenseRightArm),
			ResistPhysicalLeftLeg:   int(c.DefenseLeftLeg),
			ResistPhysicalRightLeg:  int(c.DefenseRightLeg),
...
func (s *EncounterStore) Save(enc *domain.Encounter) error {
	...
	for i, c := range enc.Combatants {
		...
		if err = qtx.InsertCombatant(ctx, dbgen.InsertCombatantParams{
			...
			DefenseHead:                       int64(c.ResistPhysicalHead),
			DefenseTorso:                      int64(c.ResistPhysicalTorso),
			DefenseLeftArm:                    int64(c.ResistPhysicalLeftArm),
			DefenseRightArm:                   int64(c.ResistPhysicalRightArm),
			DefenseLeftLeg:                    int64(c.ResistPhysicalLeftLeg),
			DefenseRightLeg:                   int64(c.ResistPhysicalRightLeg),
	...
	if len(activeCharacters) > 0 {
		...
		for _, r := range activeCharacters {
			party = append(party, domain.Combatant{
				...
				ResistPhysicalHead:      int(r.DefenseHead),
				ResistPhysicalTorso:     int(r.DefenseTorso),
				ResistPhysicalLeftArm:   int(r.DefenseLeftArm),
				ResistPhysicalRightArm:  int(r.DefenseRightArm),
				ResistPhysicalLeftLeg:   int(r.DefenseLeftLeg),
				ResistPhysicalRightLeg:  int(r.DefenseRightLeg),
...
func (s *EncounterStore) CreateCampaign(campaignID, name, startDate string, players []domain.NewCampaignPlayer) (*domain.Campaign, error) {
	...
	for _, p := range players {
		...
		if err = qtx.InsertPlayerCharacter(ctx, dbgen.InsertPlayerCharacterParams{
			...
			DefenseHead:                       int64(p.Character.ResistPhysicalHead),
			DefenseTorso:                      int64(p.Character.ResistPhysicalTorso),
			DefenseLeftArm:                    int64(p.Character.ResistPhysicalLeftArm),
			DefenseRightArm:                   int64(p.Character.ResistPhysicalRightArm),
			DefenseLeftLeg:                    int64(p.Character.ResistPhysicalLeftLeg),
			DefenseRightLeg:                   int64(p.Character.ResistPhysicalRightLeg),
