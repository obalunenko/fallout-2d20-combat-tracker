...
| - (Foreign key ID: 1)                | FOREIGN KEY | FOREIGN KEY (player_id) REFERENCES players (id) ON UPDATE NO ACTION ON DELETE CASCADE MATCH NONE               |
...
| Name                                             | Definition                                                                                                                            |
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| idx_player_characters_player_active_availability | CREATE INDEX idx_player_characters_player_active_availability<br />ON player_characters(player_id, active, availability_status, name) |
...
| trg_player_characters_player_update_keeps_combatants_consistent | CREATE TRIGGER trg_player_characters_player_update_keeps_combatants_consistent<br />BEFORE UPDATE OF player_id ON player_characters<br />WHEN EXISTS (<br />    SELECT 1<br />    FROM combatants c<br />    JOIN encounters e ON e.id = c.encounter_id<br />    JOIN players p ON p.id = NEW.player_id<br />    WHERE c.player_character_id = NEW.id<br />      AND p.campaign_id <> e.campaign_id<br />)<br />BEGIN<br />    SELECT RAISE(ABORT, 'player character player update would mismatch linked combatants');<br />END |
